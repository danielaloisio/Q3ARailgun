...
} __attribute__((aligned(16))) memoryblock_t;
...
#endif //MEMDEBUG
{
  ...
  assert(botimport.GetMemory); // bk001129 - was NULL'ed
	ptr = botimport.GetMemory(size + sizeof(memoryblock_t) + sizeof(void*) + 15);
	block = (memoryblock_t *) ptr;
	block->id = MEM_ID;
	{
		char *after_hdr = (char *)ptr + sizeof(memoryblock_t) + sizeof(void*);
		char *aligned = (char *)(((intptr_t)after_hdr + 15) & ~(intptr_t)15);
		*((memoryblock_t **)(aligned - sizeof(void*))) = block;
		block->ptr = aligned;
	}
	block->size = size + sizeof(memoryblock_t) + sizeof(void*) + 15;
#ifdef MEMDEBUG
	block->label = label;
	block->file = file;
	block->line = line;
...
#endif //MEMDEBUG
{
	...
	ptr = botimport.HunkAlloc(size + sizeof(memoryblock_t) + sizeof(void*) + 15);
	block = (memoryblock_t *) ptr;
	block->id = HUNK_ID;
	{
		char *after_hdr = (char *)ptr + sizeof(memoryblock_t) + sizeof(void*);
...
memoryblock_t *BlockFromPointer(void *ptr, char *str)
{
	...
	block = *((memoryblock_t **)((char *)ptr - sizeof(void*)));
...
#endif //MEMDEBUG
{
	...
	ptr = botimport.GetMemory(size + 16 + 15);
	if (!ptr) return NULL;
	aligned = (char *)(((intptr_t)((char *)ptr + 16) + 15) & ~(intptr_t)15);
	*((void **)(aligned - 16)) = ptr;
	*((unsigned long int *)(aligned - 8)) = MEM_ID;
	return aligned;
...
#endif //MEMDEBUG
{
	...
	ptr = botimport.HunkAlloc(size + 16 + 15);
	if (!ptr) return NULL;
	aligned = (char *)(((intptr_t)((char *)ptr + 16) + 15) & ~(intptr_t)15);
	*((void **)(aligned - 16)) = ptr;
	*((unsigned long int *)(aligned - 8)) = HUNK_ID;
	return aligned;
...
void FreeMemory(void *ptr)
{
	...
	memid = *((unsigned long int *) ((char *) ptr - 8));
	orig  = *((void **)            ((char *) ptr - 16));

	if (memid == MEM_ID)
	{
		botimport.FreeMemory(orig);
	} //end if
} //end of the function FreeMemory
//===========================================================================
//
